...
(() => {
  ...
  function formSubmitEvent(element, properties, identifyProperties) {
    ...
    [...formElements].forEach((fe) => {
      ...
      if (formElement !== element) {
        const name = formElement?.name.toLowerCase() !== "" && formElement?.name.toLowerCase() ? formElement?.name.toLowerCase() : formElement?.id.toLowerCase() !== "" && formElement?.id.toLowerCase() ? formElement?.id.toLowerCase() : void 0;
        let value;
        if (formElement?.type === "checkbox" && !formElement.checked) value = void 0;
        else if (formElement?.type === "radio" && !formElement.checked) value = void 0;
  ...
  function getCorrectWriteKey(prodWriteKey, devWriteKey, stagingDomain2, isDev2 = false) {
    if (isDevEnvironment(stagingDomain2) && devWriteKey && isDev2) {
      return devWriteKey;
    }
    return prodWriteKey;
  }
  ...
  function loadSegmentConsentManager(prodWriteKey, alwaysRequireConsent = "true", devWriteKey, stagingDomain2, productionDomain2, isDev2 = false) {
    if (alwaysRequireConsent === "false" && !prodWriteKey && !devWriteKey) {
      return;
    }
    const intendedWriteKey = getCorrectWriteKey(prodWriteKey, devWriteKey, stagingDomain2, isDev2);
  ...
  function loadSegmentAnalytics(prodWriteKey, enableConsentManager2 = "true", devWriteKey, stagingDomain2, productionDomain2, isDev2 = false) {
    ...
    if (!analytics.initialize)
      if (analytics.invoked) throw new Error("Segment snippet included twice.");
        ...
        analytics._writeKey = getCorrectWriteKey(prodWriteKey, devWriteKey, stagingDomain2, isDev2);
        analytics.SNIPPET_VERSION = "5.2.0";
        if (enableConsentManager2 === "false")
          analytics.load(getCorrectWriteKey(prodWriteKey, devWriteKey, stagingDomain2, isDev2));
        analytics.page();
      }
  }
  ...
  var segmentProductionWriteKey = scriptTag?.getAttribute("segment-prod-write-key") ?? void 0;
  ...
  var isDev = scriptTag?.getAttribute("is-dev") === "true";
